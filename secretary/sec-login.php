...
<?php
    ...
    if(isset($_POST['s_login']))
    {
      ...
      $s_pwd=($_POST['s_pwd']);//double encrypt to increase security
      $stmt=$mysqli->prepare("SELECT s_email, s_pwd, s_id FROM ams_secretary WHERE s_email=? and s_pwd=? ");//sql to log in user
      $stmt->bind_param('ss',$s_email,$s_pwd);//bind fetched parameters
      $stmt->execute();//execute bind
      $stmt -> bind_result($s_email,$s_pwd,$s_id);//bind result
      $rs=$stmt->fetch();
      $_SESSION['s_id']=$s_id;//assaign session to passenger id
                ...
                <form method ="POST">
                  <div class="login-form ">
                    ...
                    <div class="form-group">
                      <input class="form-control" name="s_email" type="text" placeholder="Email" autocomplete="off">
                    </div>

                    <div class="form-group">
                      <input class="form-control" name="s_pwd" type="password" placeholder="Password">
                    </div>
                    ...
                    <div class="form-group row login-submit">
                      <div class="col-6"><input type = "submit" name ="s_login" class="btn btn-primary btn-xl" value ="Log In"></div>
                    </div>
